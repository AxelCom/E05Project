...
public class FenetreAccueil extends JFrame implements ActionListener {
	...
	public FenetreAccueil() {
		...
		String[] tab  = ctrl_catalogue.AfficherNomsCatalogues();
		modifierListesCatalogues(tab);
		String[] tab2 = ctrl_catalogue.afficherDetailsCatalogues();
		modifierDetailCatalogues(tab2);
		modifierNbCatalogues(ctrl_catalogue.getNbCatalogue());
		setVisible(true);
	}
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource() == btAjouter)
		{
			...
			{
				ctrl_catalogue.ajouterCatalogue(texteAjout);
				modifierListesCatalogues(ctrl_catalogue.AfficherNomsCatalogues());
				modifierDetailCatalogues(ctrl_catalogue.afficherDetailsCatalogues());
				modifierNbCatalogues(ctrl_catalogue.getNbCatalogue());
		...
		if (e.getSource() == btSupprimer)
		{
			...
			{
				ctrl_catalogue.supprimerCatalogue(texteSupprime);
				modifierListesCatalogues(ctrl_catalogue.AfficherNomsCatalogues());
				modifierDetailCatalogues(ctrl_catalogue.afficherDetailsCatalogues());
				modifierNbCatalogues(ctrl_catalogue.getNbCatalogue());
			}
